...
static int ws_service_callback(struct lws* wsi, enum lws_callback_reasons reason, void* user, void* in, size_t len) {
  switch(reason) {
    ...
    case LWS_CALLBACK_CLIENT_RECEIVE:
      ...
      if(writeable_flag) destroy_flag = 1;
...
static void* pthread_routine(void* tool_in) {
  ...
  while(!connection_flag) usleep(1000 * 20);

  //*Send greeting to server*/
  printf(KBRN
         "[pthread_routine] Server is ready. send a greeting message to "
         "server.\n" RESET);
  ...
  printf(KBRN "------------------------------------------------------\n" RESET);
  sleep(1);
  // printf(KBRN"[pthread_routine] sleep 2 seconds then call
  // onWritable\n"RESET);
...
int main(void) {
  ...
  wsi = lws_client_connect(context, "localhost", 5000, 0, "/", "localhost:5000", NULL, protocol.name, -1);
  if(wsi == NULL) {
