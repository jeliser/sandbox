...
There is some combination of all these mechanisms that will allow me to have a captured lambda function sent in
    as a function pointer to the signal handler.
...
pid_t spawn(const std::string& cmd, const std::string& working_dir, ErrorCode& ec) {
  ...
  if(cpid != 0) {
    ...
    std::cout << l1(1, 2) << "  " << f1.target<int (*)(int, int)>() << "  " << b1.target<int (*)(int, int)>()
              << std::endl;
...
#endif
#if 1
    // auto lambda = [ &gcpid ] (int, siginfo_t* info, void*) -> void { gcpid = info->si_value.sival_int; printf("Got
    // here: %d\n", gcpid); }; auto func   = std::bind(lambda, std::placeholders::_1, std::placeholders::_2,
    // std::placeholders::_3); auto func   = std::function<void (int, siginfo_t*, void*)>(std::bind(lambda,
